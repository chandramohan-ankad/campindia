...
router.get('/:id', function(req, res) {
	campground.findById(req.params.id).populate('comments').exec(function(err, foundcampground) {
		if (err || !foundcampground) {
			...
			campground.find({ 'address.state': foundcampground.address.state }, function(err, relatedCampgrounds) {
				if (err || !foundcampground) {
					req.flash('error', 'Campground not found');
					return res.redirect('/campgrounds');
				} else {
					user.findById(foundcampground.author.id, function(err, founduser) {
						if (err) {
							...
							res.render('show', {
								...
								user: founduser,
								campgrounds: relatedCampgrounds
							});
						}
					});
				}
...
router.put('/:id/edit', middleware.checkCampgroundWonership, upload.single('image'), function(req, res) {
	campground.findById(req.params.id, async function(err, campground) {
		if (err) {
			...
			campground.address.city = req.body.city;
			campground.address.postalCode = req.body.postalCode;
